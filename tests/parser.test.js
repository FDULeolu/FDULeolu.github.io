...
check('meta.keywords has 6 entries', meta.keywords.split('·').length === 6, meta.keywords);
...
check('7 sections', sections.length === 7, String(sections.length));
check('section types', types === 'about,news,timeline,timeline,publications,columns,contact', types);
const slugs = sections.map(s => s.slug).join(',');
check('slugs', slugs === 'about,news,education,experience,publications,honors-service,contact', slugs);
...
/* ---- education ---- */
const edu = sections[2];
check('education: nav=no', edu.directives.nav === 'no');
